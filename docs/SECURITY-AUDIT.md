...
> 版本：對應 git branch `claude/verify-content-ZxftO`
> 評估日：2026-05-14
...
| 控制 | 實作位置 | 驗證 |
|---|---|---|
| R1 JWT 身分驗證 | `server/index.js:19,38` `requireAuth`；`server/index.js:188-197` Socket.io handshake | 受保護路由要求 `Authorization: Bearer` |
| R1 密碼雜湊 | `server/index.js:171` `bcrypt.hash(password, 12)` | bcrypt 12 rounds |
...
| R4 輸入消毒 | `server/index.js:123` `sanitizeBody` | 拒 `$` key、限長度 5000、URL 協定白名單 |
| R5 CORS 白名單 | `server/index.js:24,33,39` 由 `CORS_ORIGIN` 控制 | 非萬用字元 |
| R6 速率限制（新） | `server/index.js:55-71` `authLimiter` / `apiLimiter` | 登入 5/15min、API 300/5min |
| R7 HTTP 安全標頭（新） | `server/index.js:37` `helmet`；`nginx.conf:18-26` HSTS/CSP/XFO/XCTO/Referrer/Permissions | 全部 `always` |
| R8 備份 | `backup.sh` mongodump + 30 天輪替 | 每日 cron 建議排程 |
| 稽核日誌（新） | `server/index.js:44-51` `logAudit`；morgan combined | JSON 一行 / event |
...
| # | 風險 | Annex A | 證據 | 建議補強 |
|---|---|---|---|---|
| M1 | 無資源擁有權檢查：任何登入者可讀寫他人所有資料 | A.5.15 / A.8.2 特權存取 | `server/index.js:140-184` `models[col].find({})` 無 user filter | schema 加 `owner: ObjectId(User)`；CRUD 加 `userId` filter；UI 加團隊共享開關 |
...
| L2 | 上傳影像僅前端 `accept="image/*"`，後端不驗 magic bytes | A.8.26 應用安全 | `server/index.js:20` `express.json({ limit:'50mb' })` 無內容驗證 | `file-type` magic bytes 驗 jpg/png/webp；拒 SVG |
...
| A.5.15 | 存取控制 | ⚠️ | 全使用者皆同權限；待 M1 RBAC 補強 |
...
| A.8.2 | 特權存取 | ❌ | Mongo 共用 root；建議 per-app user |
...
### Phase 2（建議分別建立 issue / PR）

| 優先 | 項目 | 估計工作量 | 相依 |
|---|---|---|---|
| 高 | M1 RBAC（schema + UI） | 3-5 天 | 需要 UI 設計討論 |
...
| 日期 | 版本 | 摘要 |
|---|---|---|
| 2026-05-14 | 1.0 | 首版審查報告；Phase 1 補強同步合併 |
